        ...
        if(A[0].length == B.length){ //untuk perkalian gunakan panjang kolom A = panjang baris B
            ...
            for(int i=0; i<A.length; i++) { // Baris A = 2; i<2;0<2=>T lanjut looping dalam
                for(int j=0; j<B[0].length; j++){ //Kolom B = 2; j<2 => T lanjut looping dalam lagi
                    for(int k=0; k<A[0].length; k++){ //Kolom A=3;k<3=>T lanjut ke proses C, k++=1; 1<3 T; lanjut proses c; k=2;k<3 T; sampai k+3<3 F, kembali ke perulangan dalam yang j, begitu seterusnya hingga j<2=> F, maka akan ke perulangan paling luar yang i dan akan terus ngeloop smpai ke K 
